  ...
  return (
    <div className={cn("flex flex-col gap-2 w-full", className)}>
      {items.map((item, idx) => (
        <a
          onClick={onItemClick}
          className="px-4 py-3 text-center text-lg font-medium text-neutral-700 dark:text-neutral-300 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-lg transition-colors duration-200"
          key={`mobile-link-${idx}`}
          href={item.link}
        >
          {item.name}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={cn(
            "w-full flex flex-col items-center justify-center gap-4 mt-4 py-4 bg-white/90 rounded-lg shadow-lg dark:bg-neutral-900/90 backdrop-blur-sm",
            className,
          )}
        >
          {children}
